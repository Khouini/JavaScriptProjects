...
function whereAmI() {
  getPosition()
    .then(pos => {
      ...
      return fetch(
        `https://geocode.xyz/${lat},${lng}?geoit=json&auth=263896209563783231588x22369`
      );
    })
    .then(response => {
...
async function whereAmI2() {
  ...
  try {
    ...
    if (!responseGeo.ok)
      throw new Error('Problem with getting geolocalisation');
    const dataGeo = await responseGeo.json();
    const resp = await fetch(
      `https://restcountries.com/v3.1/name/${dataGeo.country}`
    );
    if (!resp.ok)
      throw new Error('Problem with getting the country informations');
